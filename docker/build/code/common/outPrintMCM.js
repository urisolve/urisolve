 ...
 * @param {number} C Current sources
 * @param {number} T Isolated volt sources
 */
 function outCircuitFundamentalsMCM(R, N, C, T){  
 ...
 */
 function outCircuitInfoMCM(F, Amps, totalCurrents){
 ...
 * @param {number} C Current sources
 */
function outEquationCalcMCM(R, N, C){
    ...
    htmlstr += '<div class="card-body text-secondary mt-1 mb-1">';
    str = "Mp = " + R + "- (" + N + " - 1) - " + C + "\\Leftrightarrow \\\\ \\Leftrightarrow E = " + E ;
    str = katex.renderToString(str, {throwOnError: false});
    htmlstr += '<div class="text-center" mt-2 mb-2"><span >'+ str + '</span></div>';
    htmlstr += '</div></div></div>';
...
function outMeshesMCM(branchObjs, meshesObjs){
    ...
    for(let i = 0; i < meshesObjs.length; i++){
        ...
        createMesh(meshesObjs[i], branchObjs, width, height, WindowWidth, WindowHeight, Xspacing, Yspacing, originX, originY, aux, containerId);
    }
}
 ...
 * @param {object} meshes mesh objects
 * @returns {string} HTML string
 */
 function outResultsMeshesMCM(meshes){
 ...
 * @returns {string} HTML string
 */
 function outResultsCurrentsMCM(currents){
 ...
 */
function createMesh(mesh, branchObjs, width, height, WindowWidth, WindowHeight, Xspacing, Yspacing, originX, originY, aux, containerId){
    ...
    for(let j = 0; j < mesh.branches.length; j++){  //add card for each branch
        createBranchBlock(svg, branchObjs[mesh.branches[j]-1], j*(width+Xspacing)+Xspacing+1+originX, 1+originY, width, height);
 ...
 */
function createBranchBlock(svg, branch, x, y, w, h){
